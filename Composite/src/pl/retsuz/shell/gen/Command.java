...
public abstract class Command implements ICommand {
    ...
    public Command(String prefix, IContext ctx, ICommand next, String pattern, ICommandVariation def, String info) {
        this.context = ctx;
        this.prefix = prefix;
        this.next = next;
        this.generalPattern = Pattern.compile(prefix + pattern + " *([a-zA-Z0-9.l\\/_]*)");
        this._default = def;
        this.man = info;
    }
    ...
    public void perform(String command) throws Exception {
        if (!checkPrefix(command)) {
            ...
            if (m.find()) {
                if (this instanceof BiCommandParameters)
